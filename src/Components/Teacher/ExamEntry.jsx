...
// const ExamEntry = () => {
//   const { regNo ,className} = useParams();
...
//             where('className', '==', className),
//             where('isStream', '==', exam.isStream || false),
//             ...(exam.isStream ? [where('stream', '==', studentData.stream)] : [])
//           );
//           const subjectSnapshot = await getDocs(subjectQuery);
//           if (subjectSnapshot.empty) {
//             setError(`No subjects found for Class ${className}${exam.isStream ? ` (${studentData.stream})` : ''}.`);
//             setSubjects([]);
//             setScores({});
//             console.warn(`No subjects found for className: ${className}, isStream: ${exam.isStream}, stream: ${studentData.stream || 'N/A'}`);
//             return;
//           }
...
//       setScores({});
//     }
//   }, [selectedExam, availableExams, editingResultId, studentData, className]);
...
//       <h2 className="text-xl font-semibold text-gray-800 mb-6">
//         Enter Exam Scores for Reg No: <span className="text-blue-700">{regNo}</span> (Class: {className}
//         {studentData?.isStream ? ` ${studentData.stream.charAt(0).toUpperCase() + studentData.stream.slice(1)}` : ''})
//       </h2>
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Validate stream for +1 or +2
        if (isHigherClass(className) && (!isStream || !stream)) {
          setStudentError(`Invalid stream configuration for Class ${className}.`);
          setLoadingExams(false);
          setLoadingResults(false);
          return;
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const examData = {
        ...
        examName: exam.name,
        regNo,
        className, // Store className in examData
        subjects: subjects.map((subject) => ({
          name: subject.name,
          score: Number(scores[subject.name]),
        })),
